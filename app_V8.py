...
st.write("Symmetrical Biometric Analysis Interface (Anti-Hallucination Active)")
...
# Initialize a persistent memory tracking buffer in Streamlit's state cache
...
def get_pose_engine(comp, is_static):
    return mp_pose.Pose(
        ...
        min_detection_confidence=0.6, # Bumped up slightly to discard hallucinations early
        min_tracking_confidence=0.6
    )

def smooth_landmark_points(current_pts, buffer_size=6):
    """
    Averages out coordinates over the last few frames to eliminate 
    shaking and high-frequency noise caused by poor camera angles.
    """
    history = st.session_state.history_buffer
    smoothed_pts = {}
    
    for joint in JOINTS.keys():
        if joint in current_pts:
            if joint not in history:
                history[joint] = []
            history[joint].append(current_pts[joint])
            
            # Keep only the most recent frames
            if len(history[joint]) > buffer_size:
                history[joint].pop(0)
                
            # Calculate the mathematical average position (mean coordinates)
            arr = np.array(history[joint])
            avg_x = int(np.mean(arr[:, 0]))
            avg_y = int(np.mean(arr[:, 1]))
            smoothed_pts[joint] = (avg_x, avg_y)
        else:
            # If the joint disappears from view, gradually decay history
            if joint in history and len(history[joint]) > 0:
                history[joint].pop(0)
...
def analyze_frame(frame, model_name, engine, target_size=(640, 480)):
    ...
    if results.pose_landmarks:
        ...
        # SENSITIVITY FILTER: Bumped visibility limit from 0.5 to 0.65 to drop low-certainty hallucinations
        raw_pts = {k: (int(landmarks.landmark[v].x * w), int(landmarks.landmark[v].y * h)) 
                   for k, v in JOINTS.items() for landmarks in [results.pose_landmarks] 
                   if landmarks.landmark[v].visibility > 0.65}

        # Apply our new Temporal Moving Average Filter
        pts = smooth_landmark_points(raw_pts)

        if "MediaPipe" in model_name:
            # Custom draw loop utilizing smoothed coordinates instead of default raw structures
        ...
        # Render the stabilized joint tracking nodes
